...
#         latent_dim = y_train.shape[1]
    ...
    # Load weights of AE
    autoencoder.load_weights('../process/AE/{}_AE_{}-{}-{}'.format(data_name, input_dim, hid_dim, latent_dim))
    ...
    # Create directories
    os.makedirs('../process/models', exist_ok=True)
    os.makedirs('../process/outcomes', exist_ok=True)
    
    # Save models as pickle file
    print("- Save outcome...")
    with open('../process/models/{}_l{}_t{}_f{}_d{}.pickle'.format(data_name, latent_dim, trees_per_label, max_features, max_depth),'wb') as file:
        pickle.dump(models, file)

    # Save evaluation as csv file
    with open('../process/outcomes/{}.csv'.format(data_name), 'a') as file:
